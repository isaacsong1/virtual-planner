...
class JournalById(Resource):
    ...
    def post(self, id):
        if journal := db.session.get(Journal, id):
            if any(entry.journal_id == id for entry in journal.entries):
                ...
                try:
                    ...
                    db.session.commit()
                    if (
                        latest_entry.date.day == new_entry.date.day
                        and latest_entry.date.month == new_entry.date.month
                        and latest_entry.date.year == new_entry.date.year
                    ):
                        db.session.delete(new_entry)
                        db.session.commit()
                        return {
                            "error": "A journal entry already exists for today"
                        }, 400
                    else:
    ...
    def patch(self, id):
        if journal := db.session.get(Journal, id):
            ...
            latest_entry = db.session.get(Entry, latest_entry_id)
            if (
                latest_entry.date.month == datetime.now().month
                and latest_entry.date.day == datetime.now().day
            ):
                try:
                    data = request.json
                    entry_schema.validate(data)
